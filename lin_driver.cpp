...
lin_driver::lin_driver() { lin_driver(defaultBaudrate); }

lin_driver::lin_driver(uint32_t baudrate) { Serial1.begin(baudrate); }
...
int lin_driver::writeMessge(uint8_t messageId, uint8_t *data,
                            size_t data_size) {
  uint8_t rxBuffer[8];
...
uint8_t lin_driver::calcIdentParity(uint8_t ident) {
  uint8_t p0 = bitRead(ident, 0) ^ bitRead(ident, 1) ^ bitRead(ident, 2) ^
               bitRead(ident, 4);
  uint8_t p1 = ~(bitRead(ident, 1) ^ bitRead(ident, 3) ^ bitRead(ident, 4) ^
                 bitRead(ident, 5));
  return (p0 | (p1 << 1)) << 6;
}
...
uint8_t lin_driver::calcEnhancedChecksum(uint8_t pID, uint8_t data[],
                                         size_t data_size) {
...
void lin_driver::printLinRxDataBufferChar(uint8_t channel, uint8_t *data,
                                          size_t dataLength) {
  int i = 0;
